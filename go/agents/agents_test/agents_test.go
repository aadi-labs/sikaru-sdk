...
func TestAgentsCreateManagedSessionWithWireMock(
	t *testing.T,
) {
	...
	_, invocationErr := client.Agents.CreateManagedSession(
		context.TODO(),
		"project_id",
		"agent_id",
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestAgentsCreateManagedSessionWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestAgentsCreateManagedSessionWithWireMock", "POST", "/v1/projects/project_id/agents/agent_id/sessions", nil, 1)
}
