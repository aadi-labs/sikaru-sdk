...
func TestReleaseWatchesCreateReleaseWatchWithWireMock(
	t *testing.T,
) {
	...
	_, invocationErr := client.ReleaseWatches.CreateReleaseWatch(
		context.TODO(),
		"project_id",
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestReleaseWatchesCreateReleaseWatchWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestReleaseWatchesCreateReleaseWatchWithWireMock", "POST", "/v1/projects/project_id/release-watches", nil, 1)
}
