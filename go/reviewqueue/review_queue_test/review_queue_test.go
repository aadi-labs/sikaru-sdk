...
func TestReviewQueueCreateReviewQueueItemWithWireMock(
	t *testing.T,
) {
	...
	_, invocationErr := client.ReviewQueue.CreateReviewQueueItem(
		context.TODO(),
		"project_id",
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestReviewQueueCreateReviewQueueItemWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestReviewQueueCreateReviewQueueItemWithWireMock", "POST", "/v1/projects/project_id/review-queue", nil, 1)
}
