...
describe("ExecutionSessionsClient", () => {
    ...
    test("spend (1)", async () => {
        ...
        server
            .mockEndpoint()
            .get("/v1/projects/project_id/execution-sessions/session_id/spend").respondWith()
            .statusCode(200).jsonBody(rawResponseBody)
                .build();

        
                        
                                const response = await client.executionSessions.spend("project_id", "session_id");
                                expect(response).toEqual(rawResponseBody);
    ...
    test("spend (2)", async () => {
        ...
        server
            ...
            await expect(async () => {
                return await client.executionSessions.spend("project_id", "session_id")
            }).rejects.toThrow(Sikaru.UnprocessableEntityError);
    });
