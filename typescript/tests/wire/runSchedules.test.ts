...
describe("RunSchedulesClient", () => {
    ...
    test("schedule_history (1)", async () => {
        ...
        server
            .mockEndpoint()
            .get("/v1/projects/project_id/run-schedules/schedule_id/occurrences").respondWith()
            .statusCode(200).jsonBody(rawResponseBody)
                .build();

        
                        
                                const response = await client.runSchedules.scheduleHistory("project_id", "schedule_id");
                                expect(response).toEqual(rawResponseBody);
    ...
    test("schedule_history (2)", async () => {
        ...
        server
            ...
            await expect(async () => {
                return await client.runSchedules.scheduleHistory("project_id", "schedule_id")
            }).rejects.toThrow(Sikaru.UnprocessableEntityError);
    });
